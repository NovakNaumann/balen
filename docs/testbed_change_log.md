...
- Replaced the clunky rectangle-based ground authoring workflow with a JSON-driven map system: terrain is now painted cell-by-cell in `balen/maps/crossroads_plaza.json`, runtime loads that map directly, and `balen/scenes/tools/map_builder.tscn` provides the side-builder workflow for terrain painting, ring/ellipse paths, sidewalk generation, and placed structures.
- Improved the map builder with a clear resizable boundary frame, object selection, editable object elevation/height, and runtime-scale 2.5D previews for buildings, tents, markers, and combat-area footprints.
- Replaced generic building-box presets with Aethelgard-specific structure previews for merchant houses, taverns, curio shops, guildhalls, and arcade rows, including blue slate roofs, gold trim, banners, dormers, spires, storefront awnings, and matching runtime rendering.
